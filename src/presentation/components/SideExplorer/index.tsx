...
import {EditOutlined, CloudDownloadOutlined, DeleteOutlined, FileAddOutlined, FolderAddOutlined } from '@ant-design/icons'
...
export function File({ file, openFile }: FileProps) {
  ...
  const { deleteFile, downloadFile, renameFile } = useFileAdapter(file)
  ...
  const renameThisFile = async (event: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
    event.stopPropagation()
    event.preventDefault()
    const newName = prompt(`Rename File: ${file.name}`, file.name)
  ...
  return (
    <div className={style.file}>
      ...
      >
        ...
        <div className={style.right}>
          <EditOutlined  className={style.iconButton} title={`Rename ${file.name}`} onClick={renameThisFile} />
...
export function Folder({ folder, openFile }: FolderProps) {
  ...
  const renameThisFolder = async (event: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
    event.stopPropagation()
    event.preventDefault()
    const newName = prompt(`Rename File: ${folder.name}`, folder.name)
    if (newName && newName.trim() !== '' && newName !== folder.name) {
      renameFolder(newName.trim())
    }
  }
